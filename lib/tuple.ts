export type ShiftTuple<TTuple extends any[]> = TTuple extends [TTuple[0], ...infer TRest] ? TRest : never;
...
export class TupleSet<TTuple extends any[]> {
    ...
    *entries(): IterableIterator<[TTuple, TTuple]> {
        for (const [value1, value2] of this.#set.entries()) {
            yield [JSON.parse(value1), JSON.parse(value2)] as [point: TTuple, point: TTuple];
        }
    }

    forEach(callbackfn: (point: TTuple, point2: TTuple, points: TupleSet<TTuple>) => void, thisArg?: any) {
